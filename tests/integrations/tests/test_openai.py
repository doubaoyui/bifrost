...
class TestOpenAIIntegration:
    ...
    def test_47_batch_list(self, openai_client, test_config, provider, model):
        ...
        response = client.batches.list(limit=10, extra_query={"provider": provider, "model": model, "role_arn": role_arn})
    ...
    def test_50_batch_e2e_file_api(self, openai_client, test_config, provider, model):
        ...
        if provider == "bedrock":
            pytest.skip("Bedrock batch E2E test requires separate handling")
        ...
        if provider == "anthropic":
            ...
            try:
                ...
                for i in range(max_polls):
                    ...
                    if retrieved_batch.status in ["completed", "failed", "expired", "cancelled", "ended"]:
                ...
                print(f"Success: Inline batch E2E completed for batch {batch.id} (provider: {provider})")
